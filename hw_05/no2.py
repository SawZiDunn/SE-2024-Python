...
def print_calendar():
    ...
    i = 0
    while i < 12:
        # if 4 tables are printed, go to the next line
        if i in [4, 8]: 
            turtle.penup()
        ...
        days = 1
        row = 0
        while row < 7:  # for loops would be more preferable
            col = 0
            while col < 7:
                ...
                if row == 0:
                    ...
                    turtle.pendown()
                    turtle.write(f"{days_each_week[col]}", align="center", font=("Ariel", 10, "normal"))
                else:
                    turtle.goto(x + 10, y - box_height / 2 - 7)
                    turtle.pendown()
                    if days <= days_each_month[i] and (col >= day_start[i] - 1 or row >= 2):
